...
class Aimsharp
{
    ...
    // implemented for player, target, focus, pet, arena1-3, party1-4, raid1-40, boss1-4, mouseover
    ...
    // implemented for "player", "arena1-3"
    // If "party1-4" or "raid1-40" is passed, will return the group member's role: "TANK", "HEALER", or "DAMAGER"
    ...
    int TotemTimer();
    // returns the time remaining on current Shadowfiend/Mindbender, Infernal, or Darkglare
    // THIS IS OBSOLETE AND WILL BE REMOVED IN A FUTURE UPDATE.

    int EnemyDR(string unit, string category);
    //unit: arena1,arena2,arena3   category: Roots, Stuns, Silences, Knockbacks, Incapacitates, Disorients
    //returns the diminishing returns level of a CC category, 0 is not DR and 3 is immuned

    int ArenaKickTimer(int ArenaNumber);
    //ArenaNumber: 1,2,3
    //returns the time remaining in milliseconds until arena unit can use their interrupt ability.
    //returns 999999 if the arena unit cannot interrupt (holy paladin etc.)
    ...
    bool TargetIsUnit(string unit);
    //returns true if target is the unit.
    //units that can be checked: focus, boss1-4, arena1-3, party1-4, player, raid1-40
    ...
    int SoulbindID();
    //returns the active soulbind ID. 0 if character does not have an active soulbind.

    int TargetExactCurrentHP();
    //returns the exact current HP of the target.

    int TargetExactMaxHP();
    //returns the exact maximum HP of the target.

    int TotemRemaining(string TotemName);
    //returns the remaining duration of TotemName.
    //The tracked totem must be added in the rotation/plugin's Initialize() method with Totems.Add(TotemName);
    //Many minions are considered totems by WoW's api. For example, Shadowfiend, Infernal, Demonic Tyrant and obviously Shaman totems are all considered totems.

    List<int> GetActiveConduits();
    //returns a List<int> of the character's active conduit and soulbind spell ID's.

    Dictionary<int,int> GetActiveConduitRanks();
    //returns a Dictionary<int,int> where the key is a conduit or soulbind spell ID and the value is the rank.

    int UnitID(string unit = "target");
    //returns the unit ID from UnitGUID()
    //works for target, focus, boss1-4

    int AlliesNearTarget();
    ...
    //EXPERIMENTAL------------------------------------------------------------------------------------------------------------------------------------------------
    int CustomFunction(string CustomFunctionName);
    ...
    //For example: CustomFunction.Add("My Character Level", "return UnitLevel('player')");
    //  will add a custom wow lua function that can be retrieved with CustomFunction("My Character Level") and returns the character's in game level.
    ...
    // Used to export objects that can be used by other plugins or rotations.  Can be used to build useful libraries.
    // Check ArenaLib plugin to see a library used to export/import useful arena information.

    object ImportObject(string PluginName, string ObjectName);
    // Used to import objects from other plugins. Can be used to build useful libraries.  
    // Check ArenaLib plugin to see a library used to export/import useful arena information.
    // The plugin being called by PluginName must be higher priority than the current calling plugin or rotation (listed higher in the list of plugins to load)

    string GetAimsharpID();
    // Grabs a ID used for rotation license generation.
